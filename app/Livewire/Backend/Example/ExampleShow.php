...
use App\Livewire\Backend\Product\Traits\WithChildPage;
use App\Livewire\Backend\Traits\GetImgUrl\WithGetImgUrl;
use App\Livewire\Backend\Product\Traits\WithFilterAndSort;
...
class ProductShow extends Component
{
    ...
    public function openEditDialog($id = 'add')
    {
        ...
        // 如果是新增就不用撈資料
        $id == 'add' ? $this->categoryTitle = '' : $this->categoryTitle = Product::find($id)->title;
        // 打開視窗
        $this->showEdit = true;
    }
    ...
    {
        // 獲取模組id
        $this->orderNum = Example::find($id)->order_num;
        // 獲取選擇id
        $this->selectId = $id;
        // 打開視窗
    ...
    public function toggleActive($id)
    {
        if (Auth::user()) {
            ...
            // 切換狀態
            $example->is_active = !$example->is_active;
            // 儲存
            $example->save();
        }
    }
    ...
    {
        return $this->filterByConditions(Product::class, withModels: ['productImage'])
            ->when(
                // 當有搜尋時
                $this->search,
                fn ($query, $search) =>
    ...
    public function render()
    {
        ...
        $this->traitsModel = Example::first();

        return view('livewire.backend.example.example-show', [
            'examples' => $this->dbms,
        ]);
    }
}
