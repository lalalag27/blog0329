<x-action-section>
    ...
    <x-slot name="description">
        {{ __('管理並註銷其他瀏覽器和設備上的活動紀錄') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-zinc-600 dark:text-zinc-300">
            {{ __('如果需要，您可以註銷所有設備上的所有其他瀏覽器紀錄。以下是您最近的一些紀錄，但此列表可能不完整。如果您認為您的帳戶已被盜用，請您立即更新您的密碼。') }}
        </div>
                        ...
                        <div>
                            @if ($session->agent->isDesktop())
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-8 h-8 text-zinc-500">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="font-semibold text-green-500">{{ __('這個設備') }}</span>
                                    @else
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('請確認您要註銷所有設備上的所有其他瀏覽器紀錄，請輸入您的密碼。') }}
